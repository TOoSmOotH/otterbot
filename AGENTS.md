...
For production-style single-port serving, run `pnpm build` and then `pnpm start` from the repo root. The backend serves the built frontend from `WEB_DIST_DIR` when `index.html` exists there.
