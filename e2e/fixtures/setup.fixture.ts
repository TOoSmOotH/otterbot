...
import { waitForServer, setSetupPassphrase, completeSetup, login, getSetupStatus } from "../helpers/api";
...
export default async function globalSetup(_config: FullConfig) {
  ...
  // Check if setup is already complete (e.g. mock mode pre-seeds everything)
  const status = await getSetupStatus();
  if (status.setupComplete) {
    console.log("[e2e setup] Setup already complete — skipping wizard.");
  } else {
    ...
    let setupCookie = cookie || "";
    if (!setupCookie) {
      // Passphrase was already set — login to get a cookie
      setupCookie = (await login(creds.setup.passphrase)) || "";
    }
  ...
  const baseUrl = process.env.BASE_URL || "https://localhost:62627";
