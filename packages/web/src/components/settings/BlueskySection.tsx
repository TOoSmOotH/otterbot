...
export function BlueskySection() {
  ...
  const service = useSettingsStore((s) => s.blueskyService);
  ...
  const [bridgeStatus, setBridgeStatus] = useState<"connected" | "disconnected" | "error">("disconnected");
  ...
  useEffect(() => {
    ...
    const handleStatus = (data: { status: "connected" | "disconnected" | "error"; handle?: string }) => {
  ...
  const handleClearCredentials = async () => {
    setSaving(true);
    await updateBlueskySettings({ identifier: "", appPassword: "" });
    setLocalIdentifier("");
    setLocalAppPassword("");
    setSaving(false);
  };
  ...
  return (
    <div className="p-5 space-y-4">
      <p className="text-xs text-muted-foreground">
        Connect Otterbot to Bluesky via the AT Protocol. Users must pair with
        the bot before it responds to their mentions or replies.
      </p>
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        <div>
          ...
          <input
            ...
            placeholder={
              credentialsSet ? "Enter new identifier to change" : "e.g. user.bsky.social"
            }
        ...
        <div>
          ...
          <input
            ...
            placeholder={service || "https://bsky.social"}
            className="w-full bg-secondary rounded-md px-3 py-1.5 text-sm outline-none focus:ring-1 ring-primary font-mono"
          />
        </div>
        ...
        {handle && (
          <div className="text-xs text-muted-foreground">
            Handle: <span className="text-foreground font-medium">@{handle}</span>
            {enabled && credentialsSet && (
              <span className={cn(
                ...
                {bridgeStatus === "connected" ? "Online" : bridgeStatus === "error" ? "Error" : "Offline"}
              </span>
            )}
          </div>
        )}
      ...
      <div className="text-[10px] text-muted-foreground space-y-1">
        <p>
          <strong>How to set up the Bluesky integration:</strong>
        </p>
        <ol className="list-decimal list-inside space-y-0.5">
          ...
          <li>Mention or reply to the bot on Bluesky</li>
