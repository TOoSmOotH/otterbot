...
export function MastodonSection() {
  ...
  const displayName = useSettingsStore((s) => s.mastodonDisplayName);
  ...
  useEffect(() => {
    ...
    const handleStatus = (data: { status: "connected" | "disconnected" | "error"; acct?: string }) => {
  ...
  useEffect(() => {
    if (instanceUrl && !localInstanceUrl) setLocalInstanceUrl(instanceUrl);
  }, [instanceUrl]);
  ...
  const handleClearToken = async () => {
    setSaving(true);
    await updateMastodonSettings({ accessToken: "" });
    setLocalAccessToken("");
    setSaving(false);
  };
  ...
  return (
    <div className="p-5 space-y-4">
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        {pairedUsers.length === 0 ? (
          <p className="text-[10px] text-muted-foreground">
            No users have been paired yet. When someone mentions the bot, they'll receive a pairing code to approve here.
          </p>
        ) : (
      ...
      <div className="text-[10px] text-muted-foreground space-y-1">
        <p>
          <strong>How to set up the Mastodon integration:</strong>
        </p>
        <ol className="list-decimal list-inside space-y-0.5">
          <li>Log in to your Mastodon instance (e.g., mastodon.social)</li>
          <li>Go to Preferences &gt; Development &gt; New Application</li>
          <li>Name it (e.g., "Otterbot") and select the required scopes: <code>read</code>, <code>write</code></li>
          <li>Submit and copy the generated access token</li>
          <li>Enter your instance URL and the access token above</li>
