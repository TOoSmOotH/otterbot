...
function ForgeAccountsSection() {
  ...
  const [form, setForm] = useState({ provider: "github" as "github" | "gitea", label: "", baseUrl: "", token: "", username: "" });
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 8 }}>
        ...
        {accounts.map((a) => (
          <span key={a.id} style={chip}>
            {a.label} ({a.provider}{a.username ? ` · ${a.username}` : ""})
            <button style={chipX} onClick={() => void del(a.id)} title="Delete account">
              <Icon icon={X} size={14} />
            </button>
          </span>
        ))}
      ...
      {open && (
        <form
          onSubmit={(e) => {
            ...
            void add(form);
            setForm({ provider: "github", label: "", baseUrl: "", token: "", username: "" });
            setOpen(false);
          }}
        ...
        >
          ...
          <input placeholder="Token" type="password" value={form.token} onChange={(e) => setForm({ ...form, token: e.target.value })} style={input} />
          <button type="submit" style={primaryBtn}>Save account</button>
        </form>
      )}
    </div>
  );
