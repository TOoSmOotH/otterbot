...
/**
 * Generate a pairing code for a Bluesky user.
 * Only one active code per user — generating a new one deletes the old.
 */
export function generatePairingCode(blueskyDid: string, blueskyHandle: string): string {
  // Remove any existing code for this user
  const existing = getConfigsByPrefix("bluesky:pairing:");
  for (const row of existing) {
    try {
      const data = JSON.parse(row.value) as PendingPairing;
      if (data.blueskyDid === blueskyDid) {
  ...
  setConfig(`bluesky:pairing:${code}`, JSON.stringify(data));
  return code;
}

/**
 * Check whether a Bluesky user is paired.
 */
export function isPaired(blueskyDid: string): boolean {
  return !!getConfig(`bluesky:paired:${blueskyDid}`);
}

/**
 * Get paired user info.
 */
export function getPairedUser(blueskyDid: string): PairedUser | null {
  const raw = getConfig(`bluesky:paired:${blueskyDid}`);
  if (!raw) return null;
  try {
...
export function approvePairing(code: string): PairedUser | null {
  const raw = getConfig(`bluesky:pairing:${code}`);
  if (!raw) return null;
  ...
  if (Date.now() - new Date(pending.createdAt).getTime() > PAIRING_TTL_MS) {
    deleteConfig(`bluesky:pairing:${code}`);
    return null;
  }

  const paired: PairedUser = {
    blueskyDid: pending.blueskyDid,
    blueskyHandle: pending.blueskyHandle,
    pairedAt: new Date().toISOString(),
  };

  setConfig(`bluesky:paired:${pending.blueskyDid}`, JSON.stringify(paired));
  deleteConfig(`bluesky:pairing:${code}`);
  return paired;
}
 ...
 */
export function revokePairing(blueskyDid: string): boolean {
  const raw = getConfig(`bluesky:paired:${blueskyDid}`);
  if (!raw) return false;
  deleteConfig(`bluesky:paired:${blueskyDid}`);
  return true;
}
...
export function listPairedUsers(): PairedUser[] {
  const rows = getConfigsByPrefix("bluesky:paired:");
...
export function listPendingPairings(): PendingPairing[] {
  const rows = getConfigsByPrefix("bluesky:pairing:");
  const now = Date.now();
