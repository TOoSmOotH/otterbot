...
export function createGameGenModelTool(ctx: ToolContext) {
  return tool({
    description:
      "Generate a 3D model (GLB format) for a game. Uses procedural generation by default " +
      "for basic shapes (cube, sphere, cylinder, plane) with color. " +
      "Experimental local 3D generation is available through a configured TRELLIS bridge. Saves to assets/models/.",
    parameters: z.object({
